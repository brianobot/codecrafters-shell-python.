...
class TestTokenizeQuote:
    ...
    def _test_tokenize_quote_with_backslash_outside_quotes(self):
        assert tokenize_quote(r""" "/tmp/file\\name" "/tmp/file\ name" """) == [
            r"/tmp/file\\name",
            r"/tmp/file\ name"
        ]
        
    def _test_tokenize_quote_with_backslash_within_single_quoted_str(self):
        assert tokenize_quote(r""" "/tmp/file/'name'" "/tmp/file/'\name\'" """) == [
            r"/tmp/file/'name'",
            r"/tmp/file/'\name\'"
        ]
        
    def _test_tokenize_quote_with_backslash_within_double_quote(self):
        assert tokenize_quote(r""" "/tmp/"file\name"" "/tmp/"file name"" """) == [
            r'/tmp/"file\name"',
            r'/tmp/"file name"',
        ]
        
    def _test_tokenize_double_quoted_str_with_backslash(self):
